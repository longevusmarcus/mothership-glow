...
const Textarea = React.forwardRef<HTMLTextAreaElement, TextareaProps>(({ className, ...props }, ref) => (
  <textarea
    className={cn(
      "flex w-full rounded-md border-none bg-transparent px-3 py-2.5 text-sm sm:text-base text-foreground placeholder:text-muted-foreground/60 focus-visible:outline-none focus-visible:ring-0 disabled:cursor-not-allowed disabled:opacity-50 min-h-[60px] sm:min-h-[44px] resize-none",
      className
    )}
    ref={ref}
    rows={1}
...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    >
      {children}
      <DialogPrimitive.Close className="absolute right-4 top-4 z-10 rounded-full bg-muted p-2 hover:bg-accent transition-all">
...
const CustomDivider: React.FC = () => (
  <div className="relative h-6 w-[1.5px] mx-1">
    <div
      className="absolute inset-0 bg-gradient-to-t from-transparent via-foreground/20 to-transparent rounded-full"
      style={{
  ...
  return (
    <>
      <PromptInput
        ...
        className={cn(
          "w-full bg-card border-border shadow-md transition-all duration-300 ease-in-out",
                ...
                {file.type.startsWith("image/") && filePreviews[file.name] && (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="absolute top-1 right-1 rounded-full bg-foreground/70 p-0.5 opacity-100 transition-opacity"
                    >
