...
const navItemsDef = [
  ...
  { id: "candidates", icon: Users, labelKey: "nav.candidates" as TranslationKey, to: "/candidates" },
  { id: "jobs", icon: Briefcase, labelKey: "nav.jobs" as TranslationKey, to: "/jobs" },
  ...
  return (
    <div className="w-[56px] bg-icon-rail flex flex-col items-center py-4 shrink-0 border-r border-detail-panel-border/50">
      ...
      <div className="mb-5">
        ...
        >
          <span className="text-[14px] font-heading font-bold text-icon-rail-active">R</span>
        </motion.div>
      </div>
      ...
      <div className="flex flex-col items-center gap-1">
        ...
        >
          <span className="text-[10px] font-body font-semibold text-icon-rail-foreground">RC</span>
        </motion.div>
      </div>
    </div>
  );
...
const AppLayout = ({ children }: AppLayoutProps) => {
  ...
  return (
    <div className="flex h-screen bg-background">
      ...
      {/* Two-level sidebar — hidden on mobile, shown via overlay */}
      <div
        className={`fixed z-50 inset-y-0 left-0 flex lg:relative lg:translate-x-0 transition-transform duration-300 ease-[cubic-bezier(0.25,1,0.4,1)] ${
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-14 flex items-center px-4 sm:px-5 border-b border-border glass sticky top-0 z-30">
          <button
            className="lg:hidden mr-3 p-1.5 transition-all active:scale-95"
          ...
          >
            <div className="flex flex-col gap-[3.5px] w-[16px]">
              <motion.span
                ...
                className="block h-[1.5px] w-full bg-muted-foreground rounded-full origin-center"
              />
              <motion.span
                animate={mobileOpen ? { opacity: 0, scaleX: 0 } : { opacity: 1, scaleX: 1 }}
                transition={{ duration: 0.15 }}
                className="block h-[1.5px] w-full bg-muted-foreground rounded-full"
              />
              <motion.span
                animate={mobileOpen ? { rotate: -45, y: -5.5 } : { rotate: 0, y: 0 }}
                transition={{ duration: 0.25, ease: [0.25, 1, 0.4, 1] }}
          ...
          <div className="flex items-center gap-1.5 ml-auto shrink-0">
            ...
            <button
              ...
              className="h-8 px-2 rounded-xl flex items-center justify-center gap-1.5 text-muted-foreground hover:text-foreground hover:bg-muted/60 transition-all"
            ...
            <button
              onClick={toggleDark}
              className="h-8 w-8 rounded-xl flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-muted/60 transition-all"
            ...
            <div className="relative" ref={notifRef}>
              <button
                ...
                className="h-8 w-8 rounded-xl flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-muted/60 transition-all relative"
              >
                ...
                {notifOpen && (
                  ...
                  >
                    ...
                    <div className="px-4 py-2 border-t border-border">
                      <button className="text-[11px] font-body font-medium text-muted-foreground hover:text-foreground transition-colors w-full text-center">
                        {t("notif.viewAll")}
                      </button>
                    </div>
                  </motion.div>
            ...
            <div className="h-8 w-8 rounded-xl bg-accent flex items-center justify-center ml-1">
              <span className="text-[10px] font-body font-semibold text-foreground/60 tracking-wide">RC</span>
            </div>
          </div>
        </header>
