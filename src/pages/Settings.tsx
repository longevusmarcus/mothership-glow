...
function PersonalOSTab({ locale }: { locale: string }) {
  const [search, setSearch] = useState("");
  const osItems = [
    { id: 1, type: "link" as const, title: "Stripe Atlas — best for incorporation", tags: ["legal", "startup"], added: "2d ago" },
    { id: 2, type: "note" as const, title: "Focus on B2B micro-SaaS under $50/mo price point", tags: ["strategy", "pricing"], added: "3d ago" },
    ...
    { id: 6, type: "note" as const, title: "Agents perform 3x better when given market context", tags: ["agents", "performance"], added: "2w ago" },
  ];
  const typeIcons: Record<string, any> = { link: PlugZap, note: Mail, doc: Shield, idea: Zap };
  ...
  return (
    <div className="space-y-5">
      <div>
        <h2 className="text-[13px] font-mondwest font-semibold flex items-center gap-2">
          <FolderOpen className="h-4 w-4 text-muted-foreground" strokeWidth={1.4} />
          Personal OS
        </h2>
        <p className="text-[11px] text-muted-foreground mt-0.5 font-mono">
          {locale === "it" ? "Il tuo spazio privato — tutto qui addestra i tuoi agenti" : "Your private space — everything here trains your agents"}
        </p>
      </div>
