...
const quickActions = [
  ...
  {
    icon: Bot,
    label: "Add agent to company",
    labelIt: "Aggiungi agente",
    prompt: "I want to add a new agent to an existing company",
  },
  { icon: Zap, label: "Deploy agent", labelIt: "Deploya agente", prompt: "I want to deploy an agent to a company" },
  {
    icon: Radio,
    label: "Browse signals",
    labelIt: "Sfoglia segnali",
    prompt: "Show me the top market signals this week",
  },
  { icon: Lightbulb, label: "Explore ideas", labelIt: "Esplora idee", prompt: "Show me pre-validated business ideas" },
  {
    icon: PlugZap,
    label: "Integrate my agent",
    labelIt: "Integra il mio agente",
    prompt: "I want to integrate my own agent",
  },
  {
    icon: BarChart3,
    label: "Performance report",
    labelIt: "Report performance",
    prompt: "Give me a performance report across all agents and companies",
  },
];
...
const Chat = () => {
  ...
  const addAssistant = (content: string, action?: ChatMessage["action"]) => {
    setMessages((prev) => [
      ...prev,
      { id: crypto.randomUUID(), role: "assistant", content, timestamp: new Date(), action },
    ]);
    setIsLoading(false);
  };
  ...
  useEffect(() => {
    ...
    if (state?.deployToCompany && !hasAutoTriggered.current) {
      ...
      setTimeout(() => {
        setMessages([
          { id: crypto.randomUUID(), role: "user", content: `Deploy agents to ${company.name}`, timestamp: new Date() },
        ]);
        setIsLoading(true);
        setTimeout(
          () =>
            addAssistant(
              `Let's deploy agents to ${company.name} (${company.type}). Pick which agents you want to activate — they'll be assigned directly to ${company.name}.`,
              "deploy_agent_flow",
            ),
          800,
        );
  ...
  const handleSend = (message: string) => {
    ...
    if (lower.includes("add") && lower.includes("agent") && (lower.includes("company") || lower.includes("existing"))) {
      setTimeout(
        () =>
          addAssistant(
            "Let's add agents to one of your companies. Pick the agent types you need, then choose the company.",
            "add_agent_to_company",
          ),
        800,
      );
      ...
      setTimeout(
        () =>
          addAssistant(
            "Let's build something. First, here are the strongest market signals I've found this week — pick the ones that resonate with you.",
            "show_signals",
          ),
        800,
      );
      ...
      setTimeout(
        () =>
          addAssistant(
            "Here are the top-scoring signals from TikTok, Reddit, Twitter/X and 10 other sources. Select the ones you want to build on.",
            "show_signals",
          ),
        800,
      );
      ...
      setTimeout(
        () =>
          addAssistant(
            "These ideas are pre-validated with revenue proof, TAM estimates, and competitor analysis. Pick one to deploy.",
            "show_ideas",
          ),
        800,
      );
      ...
      setTimeout(
        () =>
          addAssistant(
            "Let's deploy your agents. Pick which agents you want to activate and where to assign them.",
            "deploy_agent_flow",
          ),
        800,
      );
      ...
      setTimeout(
        () =>
          addAssistant("Let's get your agent connected. Choose from our agents or integrate your own.", "pick_agent"),
        800,
      );
    } else {
      setTimeout(
        () => {
          addAssistant(t(responseKeys[Math.floor(Math.random() * responseKeys.length)]));
        },
  ...
  // Flow handlers
  const handleSignalsSelected = (s: string) => {
    addUser(`Selected signals: ${s}`);
    setIsLoading(true);
    setTimeout(
      () =>
        addAssistant(
          "Great picks. Based on those signals, here are the strongest validated ideas with revenue proof. Choose one to build.",
          "show_ideas",
        ),
      900,
    );
  ...
  const handleIdeaSelected = (idea: string) => {
    ...
    setTimeout(
      () =>
        addAssistant(
          `Solid choice — "${idea}" has strong market validation. Now pick the agents that will build it.`,
          "pick_agent",
        ),
      900,
    );
  };
  const handleDeploy = (ids: string[]) => {
    const names = ids.map((id) => deployableAgents.find((a) => a.id === id)?.name || id).join(", ");
    addUser(`Deploy with: ${names}`);
    setTimeout(
      () => addAssistant(`Deploying your company with ${names}. Agents are taking over now...`, "deploying"),
      600,
    );
  };
  const handleDeployDone = () => {
    addAssistant(
      "Your company is in progress. Agents are now ready to get activated and work autonomously — check the dashboard for real-time updates.",
      "pre-deployed",
    );
  };
  const handleDeployAgentDone = (names: string, target: string) => {
    addUser(`Deploy ${names} → ${target}`);
    setTimeout(() => addAssistant(`Deploying ${names} to ${target}. Agents are spinning up now...`, "deploying"), 600);
  };
  const handleIntegrate = () => {
    addUser("I want to integrate my own agent");
    setIsLoading(true);
    setTimeout(
      () =>
        addAssistant(
          "Here's everything you need to connect your agent via REST API. Unlock full access with the MSX Pro plan.",
          "show_api_docs",
        ),
      800,
    );
  ...
  const renderExtras = (msg: ChatMessage) => {
    switch (msg.action) {
      case "show_signals":
        return <SignalsCard onSelect={handleSignalsSelected} />;
      case "show_ideas":
        return <IdeasCard onSelect={handleIdeaSelected} />;
      case "pick_agent":
        return <AgentPickerCard onDeploy={handleDeploy} onIntegrate={handleIntegrate} />;
      case "deploying":
        return <DeployingCard onDone={handleDeployDone} />;
      case "deployed":
        return <DeployedCard />;
      case "show_api_docs":
        return <ApiDocsPaywall />;
      case "deploy_agent_flow":
        return <DeployAgentCard onDone={handleDeployAgentDone} preSelectedCompany={deployToCompany} />;
      case "add_agent_to_company":
        return <AddAgentToCompanyCard onDone={handleAddAgentDone} />;
      default:
        return null;
    }
  };
              ...
              <AnimatePresence initial={false}>
                {messages.map((msg) => (
                  ...
                  >
                    ...
                    <div
                      className={`${msg.action ? "max-w-[90%]" : "max-w-[80%]"} rounded-2xl px-4 py-3 text-[13px] font-mono leading-relaxed ${msg.role === "user" ? "bg-primary text-primary-foreground" : "bg-card border border-border text-foreground"}`}
                    >
                      {msg.content}
                      {renderExtras(msg)}
                      <p
                      ...
                      >
                        {msg.timestamp.toLocaleTimeString(locale === "en" ? "en-US" : "it-IT", {
                          hour: "2-digit",
                          minute: "2-digit",
                        })}
                      </p>
