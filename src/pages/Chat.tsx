    ...
    <div className="flex flex-col h-[calc(100vh-theme(spacing.16)-theme(spacing.20))] max-w-2xl mx-auto">
      {isEmpty ? (
        <div className="flex-1 flex flex-col items-center justify-center px-4">
          <h1 className="text-[20px] font-heading font-semibold tracking-tight mb-6">
          ...
          <div className="grid grid-cols-2 sm:flex sm:flex-wrap sm:items-center sm:justify-center gap-2 w-full">
            {promptKeys.map((key) => (
              <button
                key={key}
                onClick={() => handleSend(t(key))}
                className="text-[11px] font-body px-3.5 py-2 rounded-full border border-border text-muted-foreground hover:text-foreground hover:border-foreground/20 transition-all duration-200 text-center"
              >
                {t(key)}
              </button>
            ))}
              ...
              <AnimatePresence initial={false}>
                {messages.map((msg) => (
                  ...
                  >
                    ...
                    <div
                      className={`max-w-[80%] rounded-2xl px-4 py-3 text-[13px] font-body leading-relaxed ${
                        msg.role === "user"
